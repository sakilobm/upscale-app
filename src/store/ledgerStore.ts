...
const SEED: LedgerEntry[] = [
  {
    ...
    note:           'Vacation fund advance',
    status:         'ACTIVE',
    partialReturns: [],
  },
  {
    id: 'l-2',
    personName:     'Sofia Rivera',
    personInitials: 'SR',
    personColor:    '#10B981',
    direction:      'OWED_TO_ME',
    totalAmount:    200,
    ...
    status:         'ACTIVE',
    partialReturns: [
      { id: 'pr-1', amount: 80, date: format(subDays(today, 3), 'yyyy-MM-dd'), note: 'First installment' },
    ],
  },
  {
    id: 'l-3',
    ...
    status:         'OVERDUE',
    partialReturns: [
      { id: 'pr-2', amount: 100, date: format(subDays(today, 15), 'yyyy-MM-dd'), note: 'Partial' },
    ],
  },
  {
    id: 'l-4',
    ...
    status:         'SETTLED',
    partialReturns: [
      { id: 'pr-3', amount: 120, date: format(subDays(today, 40), 'yyyy-MM-dd') },
    ],
  },
];
...
export const useLedgerStore = create<LedgerState>((set, get) => ({
  ...
  addEntry: (draft) => {
    const entry: LedgerEntry = {
      ...draft,
      id:              `l-${Date.now()}`,
      personInitials:  initials(draft.personName),
      personColor:     personColor(draft.personName),
      amountReturned:  0,
      status:          'ACTIVE',
      partialReturns:  [],
    };
  ...
  settleEntry: (entryId) => {
    set((s) => ({
      entries: s.entries.map((e) =>
        e.id === entryId
          ? { ...e, amountReturned: e.totalAmount, status: 'SETTLED' }
          : e
      ),
    }));
  },
